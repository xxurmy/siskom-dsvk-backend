...
class SyaratAdministrasiSeminarController extends Controller
{
    ...
    private const SYARAT_MAP = [
        ...
        'kartu_seminar' => [
            'url_col' => 'kartu_seminar_url', 'at_col' => 'kartu_seminar_uploaded_at',
     ...
     * berkas-seminar/{nim}_{nama}/{nama-file-tetap}
     *
     ...
     * (kartu_seminar: pdf/jpg/jpeg/png), nama file lama bisa beda ekstensi
    ...
    public function upload($seminarId, string $syaratKey, Request $request)
    {
        ...
        // Struktur folder: storage/app/private/berkas-seminar/{nim}_{nama}/
        $folderName = $this->sanitizeFolderName($seminar->nim . '_' . $seminar->nama);
        ...
        // file harus diambil lewat route ber-auth (SyaratAdministrasiSeminarController@showFile),
     ...
     * Route: GET /seminar/{id}/syarat-administrasi/{syaratKey}/file
     *
     ...
     */
    public function showFile($seminarId, string $syaratKey, Request $request)
    {
        $user = $request->user();
        if (! $user) {
            return response()->json(['message' => 'User tidak ditemukan'], 404);
        ...
        $syarat = SyaratAdministrasiSeminar::where('seminar_id', $seminar->id)->first();
        $config = self::SYARAT_MAP[$syaratKey];
